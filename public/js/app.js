...
window.DocForge = {
  ...
  bindEvents() {
    ...
    document.getElementById('expand-dedup')?.addEventListener('click', () => this.viewers?.dedup?.expandAll());
    document.getElementById('collapse-dedup')?.addEventListener('click', () => this.viewers?.dedup?.collapseAll());
    document.getElementById('search-dedup')?.addEventListener('input', (e) => this.viewers?.dedup?.search(e.target.value));

    document.getElementById('expand-module0a')?.addEventListener('click', () => this.viewers?.module0a?.expandAll());
    document.getElementById('collapse-module0a')?.addEventListener('click', () => this.viewers?.module0a?.collapseAll());
    document.getElementById('search-module0a')?.addEventListener('input', (e) => this.viewers?.module0a?.search(e.target.value));

    document.getElementById('expand-web-enrich')?.addEventListener('click', () => this.viewers?.webEnrich?.expandAll());
    ...
    document.getElementById('expand-evaluation')?.addEventListener('click', () => this.viewers?.evaluation?.expandAll());
    document.getElementById('collapse-evaluation')?.addEventListener('click', () => this.viewers?.evaluation?.collapseAll());
    document.getElementById('search-evaluation')?.addEventListener('input', (e) => this.viewers?.evaluation?.search(e.target.value));
  ...
  buildViewModel(apiResult) {
    ...
    const evaluation = stages.evaluation ? stages.evaluation.result : null;
  ...
  renderResults(data) {
    ...
    if (data.evaluation) this.renderEvaluation(data.evaluation);
  ...
  renderDedup(data) {
    const container = document.getElementById('json-viewer-dedup');
    if (!container) return;
    container.innerHTML = '';
    this.viewers = this.viewers || {};
    this.viewers.dedup = new JsonViewer(container, data, {
      collapsedDepth: 2,
      highlightInferred: false,
      showLineNumbers: true
    });
    this.viewers.dedup.render();
  },

  renderModule0A(data) {
    const container = document.getElementById('json-viewer-module0a');
    if (!container) return;
    container.innerHTML = '';
    this.viewers = this.viewers || {};
    this.viewers.module0a = new JsonViewer(container, data, {
      collapsedDepth: 2,
      highlightInferred: false,
      showLineNumbers: true
    });
    this.viewers.module0a.render();
  },

  renderWebEnrichment(data) {
    const container = document.getElementById('json-viewer-web-enrich');
    if (!container) return;
    container.innerHTML = '';
    this.viewers = this.viewers || {};
    this.viewers.webEnrich = new JsonViewer(container, data, {
      collapsedDepth: 2,
      highlightInferred: false,
      showLineNumbers: true
    });
  ...
  renderEvaluation(data) {
    const container = document.getElementById('json-viewer-evaluation');
    if (!container) return;
    container.innerHTML = '';
    this.viewers = this.viewers || {};
    this.viewers.evaluation = new JsonViewer(container, data, {
      collapsedDepth: 2,
      highlightInferred: false,
      showLineNumbers: true
    });
    this.viewers.evaluation.render();
  },
  ...
  initCarousel() {
    ...
    this.slideTitles = [
      ...
      'Normalization', 'LOV Verification', 'Mfg & Brand', 'UOM Validation', 'Fraction Conversion', 'De-duplication', 'Module 0A Pre-Dedup', 'Mfg Web Enrichment', 'Digital Assets', 'Ground Truth Evaluation', 'Taxonomy (Enrich)', 'Validation Report', 'Source Grounding',
