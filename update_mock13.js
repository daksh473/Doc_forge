...
const uomValidator = require('./server/services/uomValidator');
...
async function updateMockData() {
  ...
  for (let i = 0; i < products.length; i++) {
    const p = products[i];
    p.uom = await uomValidator.validateUOM(p.normalization || p.extraction);
  }
...
`;
  ...
  console.log('mockProducts.js updated with UOM Validation data!');
}
